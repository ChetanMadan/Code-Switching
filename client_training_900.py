...
class CodeSwitchDataset(Dataset):
    def __init__(self, lang, mode = "train", shuffle=True):
        ...
        self.csv_file = pd.read_csv(self.path + 'samples_900.tsv', header=None, sep='\t')
        self.input_length = []
        self.label_length = []
...
print(epoch_num)
writer = SummaryWriter('train_logs_900/')
...
save_model_path = os.path.join("checkpoints_900", save_model_filename)
torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
